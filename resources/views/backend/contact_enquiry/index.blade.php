									...
									<tr>
										...
										<td>
											<div class="d-flex order-actions">
 												<a href="javascript:;" class="ms-3 delete_btn" data-id="{{$enquiry->id}}"><i class='bx bxs-trash'></i></a>
											</div>
										</td>
									</tr>
									@empty
                                    <tr>
                                        <td colspan="5" class="text-center">No enquiry found.</td>
                                    </tr>
                                    @endforelse
								</tbody>
							</table>
...
<script>
    $(document).on("click", ".delete_btn", function(){
        console.log('clicked');
        let id = $(this).data("id");
        let url = "{{ route('backend.contact_enquiry.destroy')}}";
            ...
            if(result.isConfirmed){
                try{
                    ...
                    if(data.status){
                        Swal.fire(
                            "Deleted!",
                            "Enquiry has been deleted successfully.",
                            "success"
                        ).then(() => {
