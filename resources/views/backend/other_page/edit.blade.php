				...
				<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
					...
					<div class="card">
							...
							<div class="card-body p-4">
								<form class="row g-3" method="POST" action="{{route('backend.other_page.update', [$page->id])}}" enctype="multipart/form-data">
                                    @csrf
									<div class="col-md-6">
										<label for="page_name" class="form-label">Page Name<span class="text-danger">*</span></label>
										<input type="text" name="page_name" class="form-control" id="page_name" placeholder="Enter Page Name" value="{{$page->name ?? ""}}" required>
                                        @error('page_name')
                                    ...
                                    <div class="col-md-12">
										...
										<textarea class="form-control text_editor" id="content" name="content" placeholder="Content ..." rows="3" required>{{$page->content ?? "" }}</textarea> 
                                        @error('content')
                                        <p class="text-danger">{{$message}}</p>
                                        @enderror
									</div> 
                                    ...
                                    <div class="col-md-12">
										...
										<input type="text" class="form-control" id="title" name="meta_title" placeholder="Enter Meta Title" value="{{$page->meta_title ?? ""}}">
                                        @error('meta_title')
                                        <p class="text-danger">{{$message}}</p>
                                        @enderror
									</div> 
                                    <div class="col-md-6">
										...
										<textarea class="form-control" id="content" name="meta_keyword" placeholder="Enter Meta Keyword" rows="3">{{$page->meta_keyword ?? ""}}</textarea> 
                                        @error('meta_keyword')
                                        <p class="text-danger">{{$message}}</p>
                                        @enderror
									</div> 
                                    <div class="col-md-6">
										...
										<textarea class="form-control" id="content" name="meta_description" placeholder="Enter Meta Description" rows="3">{{$page->meta_description ?? "" }}</textarea> 
